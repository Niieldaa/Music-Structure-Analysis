...
# Define folder paths
folder1 = os.path.join(script_dir, "Files", "Segmentino Parsed Data", "Left")  # CSV1 folder
...
with open(output_csv, mode="w", newline="") as csv_output:
    ...
    for i in range(1, 73):
        file1 = os.path.join(folder1, f"parsed_VitalicSEGMENTINO{i}L.csv")
        file2 = os.path.join(folder2, f"parsed_VitalicQM{i}L.csv")

        if os.path.exists(file1) and os.path.exists(file2):
            print(f"\nComparing parsed_VitalicSEGMENTINO{i}L.csv with parsed_VitalicQM{i}L.csv...")
